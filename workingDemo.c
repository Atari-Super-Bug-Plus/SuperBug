...
//#define TILEMAP ((unsigned char*)0x6000)
...
/*
#define TILEMAP_BYTES 5000
unsigned char tilemap[TILEMAP_BYTES] = {
    ...
    0x44, 0x44, 0x44, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
void load_left_facing_sprite(void) {
    ...
    POKE(HPOSP0, ((sprite_x * 161) / 40) + 39);  // Main sprite
    POKE(HPOSP1, ((sprite_x * 161) / 40) + 47);  // Offset companion
}
...
void install_display_list() {
    ...
    // Top row = ANTIC Mode 2 (text mode) with LMS
...
void update_scroll() {
    ...
    if ((paddle > 201 && paddle < 228) || (paddle < 27)) {
        direction = 0; // UP
        load_sprite(car_sprite_0, 0);
...
void main(void) {
    ...
    // Optional: Define wall tile (tile 1)
    CUSTOM_CHARSET[1 * 8 + 0] = 0b10101010;
    CUSTOM_CHARSET[1 * 8 + 1] = 0b10101010;
    CUSTOM_CHARSET[1 * 8 + 2] = 0b10101010;
    CUSTOM_CHARSET[1 * 8 + 3] = 0b10101010;
    CUSTOM_CHARSET[1 * 8 + 4] = 0b10101010;
    CUSTOM_CHARSET[1 * 8 + 5] = 0b10101010;
    CUSTOM_CHARSET[1 * 8 + 6] = 0b10101010;
    CUSTOM_CHARSET[1 * 8 + 7] = 0b10101010;

    // --- Setup HUD text in row 0 ---
    memset(screen, 0, SCREEN_WIDTH);  // Optional clear
    memcpy(screen, "score", 5);  // Only if Mode 2 row is first
    memcpy(screen + 16, "gas", 3);
    ...
    // --- Load player sprite and position ---
    //clear_players();
    load_sprite(car_sprite_0, 0);
    //load_left_facing_sprite();
